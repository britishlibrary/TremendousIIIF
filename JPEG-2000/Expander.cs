             ...
             (var a, var b) = InitialiseKakaduLogging(log);
                ...
                using (var tile_dims = new Ckdu_dims())
                {
                    ...
                    while (node.exists())
                    {
                        ...
                        {
                            hasGeoData = true;
                            log.Debug("hasGeoData {@hasGeoData}", hasGeoData);
                        }
        ...
        public static (ProcessState state, SKImage image) ExpandRegion(Stream stream, ILogger Log, Uri imageUri, ImageRequest request, bool allowSizeAboveFull, C.ImageQuality quality)
        {
            ...
            using (var limiter = new Ckdu_quality_limiter(quality.WeightedRMSE))
            {
                ...
                try
                {
                    ...
                    Log.Debug("Size Source {@x} {@y}", originalWidth, originalHeight);
                    ...
                    Log.Debug("add_ilayer extracted dimension: {@AccessPos}, {@AccessSize}, {@IsEmpty}, {@Scale}",
                    ...
                    if (0 != checkScale)
                    {
                        Log.Debug("Scaling error: CheckScale {@CheckScale} Requested {@Scale} Dims {@Dims}", checkScale, scale, viewSize);
                    ...
                    compositor.get_total_composition_dims(extracted_dims);
                    Log.Debug("get_total_composition_dims extracted dimension: {@AccessPos}, {@AccessSize}, {@IsEmpty}, {@Scale}",
                        extracted_dims.access_pos(), extracted_dims.access_size(), extracted_dims.is_empty(), scale);
                    ...
                    Log.Debug("Set quality limiting: {@Limiter}, {@HorizontalPPI}, {@VerticalPPI}",
                    ...
                    Log.Debug("compositor.is_processing_complete: {@prccomp}", compositor.is_processing_complete());

                    var compositorBuffer = compositor.GetCompositionBitmap(extracted_dims);
                    if (null == compositorBuffer)
                    {
                        Log.Error("Unable to composite region");
